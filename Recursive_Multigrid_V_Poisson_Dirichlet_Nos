...
void multigrid(double ***u_new, double ***u, double ***rhs, int linhas,
               ...
               int min_colunas, int max_linhas, int max_colunas, int i);
...
// Iterações de MG
    ...
    while(k < MAX_MG){
        multigrid(u_new, u, rhs, linhas, colunas, dx, dy, min_linhas, min_colunas, Ny, Nx, i);
...
double erro_absoluto(double **u_Sol_Anal, double **u, int linhas, int colunas){
    ...
    printf("Norma infinita do erro: %.5e \n", erro_value);
    return erro_value;
}
//-------------------------------------------------------------
// Salva dados
...
void multigrid(double ***u_new, double ***u, double ***rhs, int linhas,
    ...
    if (linhas == min_linhas && colunas == min_colunas) {
        ...
        i++;
        multigrid(u_new, u, rhs, linhas, colunas, dx, dy, min_linhas, min_colunas, max_linhas, max_colunas, i);
        return;
    }
    ...
    for(int j = i; j > 0; j--) {
        ...
        prolongamento(u_new[j], interpol_support, ( (linhas - 1)/2 ) + 1, ( (colunas - 1)/2 ) + 1, linhas, colunas);

        inicializa_matriz(u_new[j], ( (linhas - 1)/2 ) + 1, ( (colunas - 1)/2 ) + 1);

        soma_matriz(u_new[j - 1], interpol_support, u_new[j - 1], linhas, colunas);

        gauss_seidel(u_new[j - 1], u[j - 1], rhs[j - 1], iter_up, dx, dy, linhas, colunas);
