...
void multigrid(double ***u_new, double ***u, double ***rhs, int linhas,
               ...
               int min_colunas, int max_linhas, int max_colunas, int i, int n_grids, int controle, int seg_pass);
...
// Iterações de MG
    ...
    while(k < MAX_MG){
        multigrid(u_new, u, rhs, linhas, colunas, dx, dy, min_linhas, min_colunas, Ny, Nx, i, n_grids, controle, seg_pass);
...
void multigrid(double ***u_new, double ***u, double ***rhs, int linhas,
    ...
    if (i == n_grids - 1) {
        ...
        //printf("Resolve: i = %d, linhas = %d, colunas = %d\n\n", i, linhas, colunas);
    }
    else {
        ...
        //printf("Restringe: i = %d, linhas = %d, colunas = %d\n\n", i, linhas, colunas);
        linhas = ( (linhas - 1)/2 ) + 1;
        colunas = ( (colunas - 1)/2 ) + 1;
        ...
        i++;
        multigrid(u_new, u, rhs, linhas, colunas, dx, dy, min_linhas, min_colunas, max_linhas, max_colunas, i,n_grids, controle, seg_pass);
        return;
    }
    int iter_interpol = 0;
    while(i > 0) {
        ...
        dx = (bx- ax) / (colunas - 1);
        dy = (by - ay) / (linhas - 1);
        //printf("Interpola: i = %d, linhas = %d, colunas = %d\n\n", i, linhas, colunas);
